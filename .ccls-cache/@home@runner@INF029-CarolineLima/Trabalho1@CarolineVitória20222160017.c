...
int validarData(DataQuebrada dataquebrada){
  if (dataquebrada.iAno > 1 && dataquebrada.iAno < 100) {// aqui é para caso digitem apenas duas casas do ano, ex: "95" 
...
DiasMesesAnos q2(char datainicial[], char datafinal[]) //quase pronta 
{
    ...
    else if(data1.iAno == data2.iAno && data1.iMes == data2.iMes && data1.iDia > data2.iDia){
        dma.retorno = 4;
        return dma;
    }
    ...
    switch (data2.iMes) {
        case 1:
            ...
            break;
        case 2:
            if (data2.iAno % 4 == 0 && (data2.iAno % 100 != 0 || data2.iAno % 400 == 0)) {
                dias += 29;
            } else {
                dias += 28;
            }
...
int q3(char *texto, char c, int isCaseSensitive)//questão pronta 
{
    ...
    for(int i=0; texto[i]!= '\0'; i++){
      ...
      else if(texto[i] == c || texto[i] == c - 32 || texto[i] == c + 32){ //32 por conta da tabela ASCII
        qtdOcorrencias++;
      }
    }
...
{
  for(int i = 0; strTexto[i] != '\0'; i++) { //remove o caractere acentuado de strBusca e strTexto movendo-os para a esquerda 
  if(strTexto[i] == -61) {
    ...
    for(int i = 0; i < strlen(strTexto); i++) {
        if(strBusca[0] == strTexto[i]) {
            ...
            if(contador == strlen(strBusca)) {
                ...
                posicoes[index+1] = i + strlen(strBusca); // armazena posição final da ocorrência
                index += 2; // incrementa o índice do vetor de posições, incremento com 2 porque a string ocupa duas posiçoes no vetor: inicial e final. Assim, é reservado 2 posições para a próxima ocorrência.
            }
        }
    }
  return qtdOcorrencias; 
...
int q6(int numerobase, int numerobusca) //questão pronta
{
  ...
  while(tmpBusca > 0){//aqui é o cálculo do número de digitos do número busca
    numDigitosBusca++;
    tmpBusca /= 10; 
  }
  while(tmpBase > 0){
    digitoBase = tmpBase % 10; //ultimo digito do numero base
    if(digitoBase == numerobusca % 10){ //verifica se é igual ao primeiro digito do numero busca 
      numDigitosBase = 1; 
    ...
    if(numDigitosBase == numDigitosBusca){//se todos os dígitos coincidirem, eu incremento a qtd de ocorrências
      qtdOcorrencias++; 
    }
